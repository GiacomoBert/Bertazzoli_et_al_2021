...
% eegdatapro_rm_ch_tr_1() - displays epoched TMSEEG data, allowing
...
%           option_num - option number of eegdatapro_rm_ch_tr step
...
S.fh = figure('menubar','none',...
              'Toolbar','none',...
              'Units','normalized',...
              'name','EEGDataPro Removing Channel and Trials',...
              'numbertitle','off',...
              'resize','off',...
              'color',backcolor,...
...
if (VARS.(sprintf('PULSE_ST_%d',VARS.option_num))==0) && (VARS.(sprintf('PULSE_END_%d',VARS.option_num))==0)
    list_attribute={'minmax','High Freq'};
else
    list_attribute={'minmax w/ TMS residual','minmax wo/ TMS residual','High Freq'};
end
...
[file, EEG]           = eegdatapro_load_step(S.step_num);
...
VARS.NUM_BAD_CHANS  = ceil(EEG.nbchan*VARS.(sprintf('PCT_BAD_CHANS_%d',option_num))/100);
VARS.NUM_BAD_TRIALS  = ceil(EEG.nbchan*VARS.(sprintf('PCT_BAD_TRIALS_%d',option_num))/100);
...
%Scatter Plot
scatter(1:N,mean(S.M),floor(400/log(N)),'ko','filled')
...
if isequal(get(gco,'type'),'scatter')
    ...
    S.ft = figure('position',[40 80 800 500],'color', backcolor);
    N = size(S.M,2);
    scatter(1:N,S.M(S.ch,:),floor(400/log(N)),'ko','filled')
    points   = flipud(findobj(get(gca,'Children'),'type','scatter'));
    set(gca,'NextPlot','add');
    label_list = {S.EEG.chanlocs.labels};
...
t_st  = find(EEG.times>VARS.(sprintf('TIME_ST_%d',VARS.option_num)),1,'first');
t_end = find(EEG.times<VARS.(sprintf('TIME_END_%d',VARS.option_num)),1,'last');
p_st  = find(EEG.times<VARS.(sprintf('PULSE_ST_%d',VARS.option_num)),1,'last');
p_end = find(EEG.times<VARS.(sprintf('PULSE_END_%d',VARS.option_num)),1,'last');

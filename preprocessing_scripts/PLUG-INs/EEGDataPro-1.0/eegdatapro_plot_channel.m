...
% eegdatapro_plot_channel() - Displays all trials within a selected channel for
...
function eegdatapro_plot_channel(S)
...
if ~state
    ...
    ylim([VARS.(sprintf('PLT_CHN_YMIN_%d',VARS.option_num)) VARS.(sprintf('PLT_CHN_YMAX_%d',VARS.option_num))]);
    xlim([floor(min(S.EEG.times)) ceil(max(S.EEG.times))]);
    hold off
end

...
switch clu_METH
    ...
    case 3 , % Boosted-Kmeans (MinMax)
        part_METH = 'KM';
        data_To_Clust = wtbxappdata('get',fig,'data_To_Clust');
        [IdxCLU,~,Centers,SUMD,D] = ...
            kmboostclust('kmeans_MM',data_To_Clust,NbCLU,kmeans_PAR{:});
        part_PAR = struct('distance',distance,'start',l_OR_s_METH);
        part_VAR = struct('Centers',Centers,'D',D,'SUMD',SUMD);
        
    case 4 , % Boosted-Kmeans (Entropy)
        part_METH = 'KM';
        data_To_Clust = wtbxappdata('get',fig,'data_To_Clust');
        [IdxCLU,~,Centers,SUMD,D] = ...
            kmboostclust('kmeans_ENT',data_To_Clust,NbCLU,kmeans_PAR{:});
        part_PAR = struct('distance',distance,'start',l_OR_s_METH);
        part_VAR = struct('Centers',Centers,'D',D,'SUMD',SUMD);
end
                ...
                for k=1:NbCLU
                    ...
                    if circleFLAG(1)
                        cirCENT(k) = plot(Y(k,1)+ rho(k)*cos(tC),...
                            Y(k,2) + rho(k)*sin(tC),':','Color',colorCENT);
                    end
                    hold on
...
if plotEXIST
    switch opt_VM
        ...
        case {2,3}
            ...
            if ~isempty(hdl_OF)
                vOF = [vOF,...
                    hdl_OF.ptnCENT,hdl_OF.txtCENT,...
                    hdl_OF.ptnSIG,hdl_OF.cirCENT];
            end

...
switch numOPT
    ...
    case 1 , [varargout{1:nbout}] = wtc_aswdr_enc(varargin{:});
        %-- 'decode' --%
    case 2 , [varargout{1:nbout}] = wtc_aswdr_dec(varargin{:});
        %--- 'save' ---%
    case 3 , [varargout{1:nbout}] = wtc_aswdr_save(varargin{:});
        %--- 'load' ---%
    case 4 , [varargout{1:nbout}] = wtc_aswdr_load(varargin{:});
end


%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++%
    function WTC_Struct =  wtc_aswdr_enc(X,wname,level,modeDWT,...
                MaxLoop,ColType,stepFLAG,CurrentAxe) %#ok<*INUSL>
        % WTC_ASWDR_ENC Main program - EZW-WDRcodes matrix X.
        ...
        Current_scan_Plan = scan_Plan_INI;
        while MoreLoop
            ...
            % 2) Significance Pass.
            %----------------------
            SigniFicant = abs(Matrice_TMP(Current_scan_Plan))>=Thres;
            idx_in_MAT  = Current_scan_Plan(SigniFicant);
            ...
            % 4) Create a new scan order.
            %----------------------------
            Signif_INDICES = LST_Val_Signif(1:PtrSignific,1);
            IdxPlan = 1+floor((Signif_INDICES-0.5)/nb_PIX);
            delta_Plan = (IdxPlan-1)*nb_PIX;
            TMP = Signif_INDICES - delta_Plan;
            II = TabFirstCHILD(TMP);
            lst_CHILD = [II , II+1 , II+rY, II+1+rY] + ...
                delta_Plan(:,ones(1,4));
            lst_CHILD(isnan(II),:) = [];
            lst_CHILD = lst_CHILD';
            lst_CHILD = lst_CHILD(:);            
            [nul,id] = setdiff(lst_CHILD,Signif_INDICES); %#ok<ASGLU>
            lst_CHILD = lst_CHILD(id);

            Current_scan_Plan_TMP = Current_scan_Plan;
            [TF,LOC_End] = ismember(Signif_INDICES,Current_scan_Plan); %#ok<ASGLU>
            [TF,LOC_Beg] = ismember(lst_CHILD,Current_scan_Plan); %#ok<ASGLU>
            Current_scan_Plan_TMP([LOC_Beg(:);LOC_End(:)]) = [];
            Current_scan_Plan = [ ...
                Current_scan_Plan(LOC_End(:)) ; ...
                Current_scan_Plan(LOC_Beg(:)) ; ...
                Current_scan_Plan_TMP   ...
                ];
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++%
    function Xrec = wtc_aswdr_dec(WTC_Struct,stepFLAG)

        CurrentAxe = [];
        nbin = nargin;
        if nbin<2 , stepFLAG = 3; end
        if ischar(WTC_Struct)
            WTC_Struct = wtcmngr('load',WTC_Struct);
        end        
        ...
        while MoreLoop
            ...
            if numLoop<=nbLoopSignif
                ...
                idx_in_MAT = Current_scan_Plan(Indices(first:last));
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++%
    function fileSize = wtc_aswdr_save(filename,WTC_Struct)
        ...
        % Begin Saving.
        %--------------
        codeID = wtcmngr('meth_ident','aswdr');
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++%
    function WTC_Struct = wtc_aswdr_load(filename)
        % File settings.
        %---------------
        tmp_filename  = def_tmpfile(filename);
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++%
    function fileSize = wtc_aswdr_save_NEW(filename,WTC_Struct) %#ok<DEFNU>
        ...
        fwrite(fid,WTC_Struct.Header.Power,'uint8');
        fwrite(fid,WTC_Struct.Header.Mean,'ubit32');
        nbCHAR = length(WTC_Struct.Header.Methode);
        fwrite(fid,nbCHAR,'ubit4');
        fwrite(fid,WTC_Struct.Header.Methode,'uint8');
        fwrite(fid,WTC_Struct.Header.Level,'uint8');
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++%
    function WTC_Struct = wtc_aswdr_load_NEW(filename)   %#ok<DEFNU>
        % File settings.
        %---------------
        tmp_filename  = def_tmpfile(filename);
        ok_TMP = exist(tmp_filename,'file');
        ...
        WTC_Struct.Header.Power = fread(fid,1,'uint8');
        WTC_Struct.Header.Mean = fread(fid,1,'ubit32');
        nbCHAR = fread(fid,1,'ubit4');
        wname  = fread(fid,nbCHAR,'uint8');
        WTC_Struct.Header.Methode = char(wname');
        WTC_Struct.Header.Level = fread(fid,1,'uint8');

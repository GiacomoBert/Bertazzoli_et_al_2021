...
%   is a string denoting the name of an orthogonal wavelet. Orthogonal
%   wavelets are designated as type 1 wavelets in the wavelet manager.
%   Valid built-in orthogonal wavelet families begin with 'haar','dbN',
%   'fkN', 'coifN', or 'symN' where N is the number of vanishing moments
%   for all families except 'fk'. For 'fk', N is the number of filter
%   coefficients. You can determine valid values for N by using waveinfo.
%   For example, waveinfo('db'). You can check if your wavelet is
%   orthogonal by using wavemngr('type',wname) to see if a 1 is returned.
%   For example, wavemngr('type','db2').
%
...
%   satisfy the conditions for an orthogonal wavelet.
%
...
%   periodically. You must enter the entire string 'reflection'. If you
%   added a wavelet named 'reflection' using the wavelet manager, you must
%   rename that wavelet prior to using this option. 'reflection' may be
%   placed in any position in the input argument list after X.
%
%
%   % Example 1:

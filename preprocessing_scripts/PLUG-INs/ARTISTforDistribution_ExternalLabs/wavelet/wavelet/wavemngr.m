...
switch option
    ...
    case 'indw'
      ...
      if ok_wave
          ...
          errargt(getWavMSG('Wavelet:moreMSGRF:Err_WavTST'),msg,'msg');
    ...
    case 'del'
        ...
        if isempty(varargin{1})
            err = 1; 
            msg = getWavMSG('Wavelet:moreMSGRF:Empty_FN_or_FSN');       
        else
        ...
        if err==0
            if isempty(i_fam)
                err = 1;
                msg = getWavMSG('Wavelet:moreMSGRF:Invalid_FamSName');
            elseif i_fam<=NB_FamInWTB
                err = 1;
                fn  = Wavelets_Info(i_fam).familyName;
                msg = getWavMSG('Wavelet:moreMSGRF:CannotDel_Wave',fn);
            end
        end
        if err
            errargt(mfilename,msg,'msg');

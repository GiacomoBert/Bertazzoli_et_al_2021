...
%DEMOLIFT Shows Lifting functions in the Wavelet Toolbox.
%
    ...
    switch action
        case 'addHelp'
            ...
            wfighelp('addHelpItem',hdlFig, ...
                getWavMSG('Wavelet:dw1dRF:HLP_WavDec'),'DW1D_DECOMPOS');
            
        case 'auto'    , wshowdrv('#autoMode',mfilename,'close');
        case 'gr_auto' , wshowdrv('#gr_autoMode',mfilename,'close');
        case 'getFigParam'
            figName  = getWavMSG('Wavelet:wavedemoMSGRF:Lifting');
            showType = 'mix10';
        ...
        case 'initShowViewer'
            ...
            pos(2) = pos(2)-hBtn;
            popstr = char('blocks','bumps','heavy sine','doppler','quadchirp','mishmash');
            txtstr = 'Signal';
            propUIC = {'Units',propAuto{1},'Position',pos,'BackgroundColor',txtBkColor};
...
if nargout<1,
    ...
    slide(idx).text = {
        ''
        getWavMSG('Wavelet:wavedemoMSGRF:Press_StartBtn','Lifting')
        ''
    ...
    slide(idx).code = {
        ...
        'set(popWAV,''Value'',1, ''String'',char(''db1'',''db2'')); '
        'set(ediCFS,''String'',''''); '
        ...
        'Infostr = char(S1,S2,S3,S4,S5,S6,S7,S8,S9,S10,S11,S12,S13,S14,S15);'
        ...
        'set(t(1),''String'',Infostr,''FontName'',''FixedWidth'',''FontSize'',10);'
        '' };
    ...
    slide(idx).code = {
        ...
        'Infostr = char(S1,S2,S3,S4,S5,S6,S7,S8,S9,S10,S11,S12,S13,S14,S15,S16,S17);'
        'set(t(1),''String'',Infostr,''FontName'',''FixedWidth'',''FontSize'',10);'
        '' };
    ...
    slide(idx).code = {
        ...
        'Infostr = char(S1,S2,S3,S4,S5,S6,S7,S8,S9,S10,S11,S12,S13,blanc,S14,S15,S16);'
    ...
    slide(idx).code = {
        ...
        'bcar = '' '';'
        'LSdb2 = liftwave(''db2'');'
        'PStr = disp(laurpoly(LSdb2{2,2},LSdb2{2,3}));'
        ...
        'S12 = [''         P'', PStr ''  ''];'
        'Infostr = char(S1,S2,S3,S4,blanc,S5,S6,S7,blanc,S8,S9,blanc,S10,S11,S12);'
    ...
    slide(idx).code = {
        ...
        'S5  = '' the Wavelet Toolbox needs two types of object:'';'
        'S7  = ''    - LAURPOLY Object to manage Laurent Polynomials.'';'
        'S8  = ''    - LAURMAT Object to manage Laurent Matrices.'';'
        'S10 = '' The last one is used in particular for the Factorization'';'
        'S11 = '' Algorithm of "usual" wavelets.'';'
        'Infostr = char(S1,S2,blanc,S4,S5,blanc,S7,blanc,S8,blanc,S10,S11);'
    ...
    idx = idx+1;
    slide(idx).code = {
        'S1  = ''LIFTING (5): laurpoly OBJECT'';'
        'S2  = ''-------------------------'';'
        'HelpSTR = help(''laurpoly''); indx = find(HelpSTR==10);'
        'jdx = indx(end-3)-1;HelpSTR = HelpSTR(1:jdx);'
        'HelpSTR = demolift(''Local_FUN'',''wlift_str_util'',''replacePOW'',HelpSTR);'
        'Infostr = char(S1,S2,HelpSTR);'
        'set(t(1),''String'',Infostr,''FontName'',''FixedWidth'',''FontSize'',9);'
        '' };
    ...
    slide(idx).text = {
        ...
        '   or click on the "Info" button '
        ' '
        '   to see more information about laurpoly OBJECT'
        ''};
    
    slide(idx).info = 'laurpoly';
    
    %========== Slide 8 ==========
    idx = idx+1;
    slide(idx).code = {
        'S1  = ''LIFTING (6): laurmat OBJECT'';'
        'S2  = ''-------------------------'';'
        'HelpSTR = help(''laurmat'');'
        'HelpSTR = demolift(''Local_FUN'',''wlift_str_util'',''replacePOW'',HelpSTR);'
        'Infostr = char(S1,S2,HelpSTR);'
        'set(t(1),''String'',Infostr,''FontName'',''FixedWidth'',''FontSize'',10);'
        '' };
    ...
    slide(idx).text = {
        ...
        '   or click on the "Info" button '
        '   '
        '   to see more information about laurmat OBJECT'
        ''};
    
    slide(idx).info = 'laurmat';
    ...
    slide(idx).code = {
        ...
        'Infostr = char(S1,S2,S3,S4,S5,S6,S7,S8,S9,S10,S11,S12,S13,S14,S15,S16,S17,S18);'
    ...
    slide(idx).code = {
        ...
        'S5  = blanc;'
        'S6  = HStr;'
        'Infostr = char(S1,S2,S3,S4,S5,S6);'
    ...
    idx = idx+1;
    slide(idx).code = {
        'G = laurpoly(1,-1)*modulate(reflect(H));'
        'GStr = disp(G);'
        ...
        'S6  = blanc;'
        'S7  = GStr;'
        'Infostr = char(S1,S2,S3,S4,S5,S6,S7);'
    ...
    slide(idx).info = 'laurpoly/modulate';
    ...
    slide(idx).code = {
        ...
        'S8  = H_EvenPart;'
        'S9  = H_OddPart;'
        'Infostr = char(S1,S2,S3,S4,S5,S6,S7,S8,S9);'
    ...
    slide(idx).code = {
        ...
        'S6  = GStr;'
        'S7  = blanc;'
        'tmpSTR_E = char(H_EvenPart,G_EvenPart);'
        'tmpSTR_O = char(H_OddPart,G_OddPart);'
        'tmpSTR_EO = [tmpSTR_E , bcar(ones(2,8)) , tmpSTR_O];';
        'Infostr = char(S1,S2,S3,S4,S5,S6,S7,tmpSTR_EO);'
    ...
    slide(idx).code = {
        ...
        'end'
        'if isequal(wname,''db2'')'
        '   tmpSTR = char([tmp{1},tmp{2}],'' '','' '',[tmp{3},tmp{4}]);'
        '   interSTR = ''none'';'
        'else'
    ...
    slide(idx).info = 'laurpoly/ppmfact';
    ...
    slide(idx).code = {
        ...
        'S9  = blanc;'
        'S10  = LS_p_STR;'
        'Infostr = char(S1,S2,S3,S4,S5,S6,S7,S8,S9,S10);'
    ...
    slide(idx).code = {
        ...
        'Infostr = char(S1,S2,S3,S4,S5,S6,S7,S8,S9,S10,S11);'
    ...
    slide(idx).code = {
        ...
        'Infostr = char(S1,S2,S3,S4,S5,S6,S7,S8,S9,S10,S11,S12,S13);'
    ...
    slide(idx).code = {
        ...
        'popstr = char(''db1'',''db2'',Lift_1,Lift_2);'
...
%--------------------------------------------------------------
function plot_phi_psi(LS,ax) %#ok<*DEFNU>
...
xlabel(getWavMSG('Wavelet:commongui:Str_ScaFun'));
axes(ax(3)); plot(psi,'g','LineWidth',2); grid
set(ax(3),'YLim',yLimPSI);

xlabel(getWavMSG('Wavelet:commongui:Str_Wavelet'));
...
switch DIM
    case '1D'
        ...
        if (idxMIN>0)
            if isempty(hdl_DEN)
                axes(ax(1)) ; hold on; plot(xREC,'y','Tag','denoSIG');
            else
    ...
    case '2D'
        ...
        axes(ax(1)) ; image(x); title(getWavMSG('Wavelet:commongui:OriImg'))
        set(ax(1),'XLim',[1,size(x,2)],'YLim',[1,size(x,1)])
        axes(ax(3));
        if isempty(LSType) , image(abs(xDEC)); else imagesc(abs(xDEC)); end
        xlabel('Original Lifted Decomposition (in place)','Color','w')
        axes(ax(4));
        if isempty(LSType) , image(abs(xDEC_Thr)); else imagesc(abs(xDEC_Thr)); end
        xlabel('Modified Lifted Decomposition (in place)','Color','w')
...
wtbxappdata('set',gcf,['analysis_PARAM_' DIM],...
    {x,xDEC,LSnew,level,ax,keptCFS,DIM});
wtbxappdata('set',gcf,'analysis_DIM',DIM);
%--------------------------------------------------------------
function edit_CB
...
keptCFS = str2double(get(gcbo,'String'));
if isempty(keptCFS)
...
%WLIFT_STR_UTIL String utilities for lifting examples.
switch option
    case 'replace'

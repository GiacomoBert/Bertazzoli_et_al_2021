function EEG = ARTISTMain(EEG,cfg, intermediatefolder, all_datasets, maincounter)  %in GIACOMO MOD: intermediatefolder, all_datasets, maincounter needed to save bad epochs file

% ARTISTMain - ARTIST algorithm for fully automated TMS-EEG artifact
%              rejection %find GIACOMO MOD for changes to adapt pipeline to
%              GIACOMO datasets
% Usage:
...
EEG.artcomp1=artcomp;
%% BANDPASS FILTER CONTINUOUS DATA BETWEEN 1 AND 90 Hz %GIACOMO MOD: lpass at 90Hz because TMSEEG preprocessing pipeline cannot do 100Hz (standardized)
disp('BANDPASS FILTER 1:90Hz');pause(1)
lcut = 1;hcut = 90;
EEG = pop_eegfiltnew(EEG, lcut, 0);
EEG = pop_eegfiltnew(EEG, 0, hcut);

%% NOTCH FILTERING TO REMOVE LINE NOISE %GIACOMO MOD: line at 50Hz (Italian line frequency)
...
EEG.arttrial=arttrial; %GIACOMO TO SAVE ART TRIALS
...
EEG.artchan=artchan; %GIACOMO TO SAVE ART CHANNELS
% % %% GIACOMO MOD: save indices of bad epochs and bad channels
% % save([intermediatefolder all_datasets{maincounter} '\bad_epochs_' all_datasets{maincounter}], 'arttrial');
% % save([intermediatefolder all_datasets{maincounter} '\bad_channels_' all_datasets{maincounter}], 'artchan');
...
% GIACOMO to put a limit on ICA comp removed to 15 maximum
if artcomp %if there are component to remove
    
    if length(artcomp) > 15 %if there are morethan 15 components to remove
        
        artcomp(16:length(artcomp))=[]; %delet all the component after the 15th and then proceed normally
        % PLOT ICA MAPS
        ARTIST_plotICA(EEG, cfg, artcomp, 2)
        
        EEG = pop_subcomp(EEG, artcomp);
    end
else %if there are no comp to remove, do everything normally
    % PLOT ICA MAPS
    ARTIST_plotICA(EEG, cfg, artcomp, 2)
    
    EEG = pop_subcomp(EEG, artcomp);
end
EEG.artcomp2=artcomp; %GIACOMO to save ART ICA COMP
%% COMMON AVERAGE REFERENCE
disp('COMMON AVERAGE REFERENCING'); pause(1)
EEG = pop_reref(EEG, []);

%% BASELINE CORRECTION w.r.t -1000 ~ -2 ms %GIACOMO MOD

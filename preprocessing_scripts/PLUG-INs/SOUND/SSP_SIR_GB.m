function [data_correct, artifact_topographies, data_clean, P,filt_ker,SSP_SIR_operator, PCs_variance] = SSP_SIR_GB(data, PC, LFM, M, badC, is_ave_ref,fs,timeAxis,TimeROI)
...
% outputfolder='C:\Users\Marta\Dropbox\SoloMe\AAA_LAVORO\Lavoro FbF\TMS connectivity response\AnalisiSound\Final\prova';
% subj   =  { 'S02S1'};
...
% x = int8(linspace(-x_scal/2, x_scal/2,x_scal)); %GIACOMO commented

i=1:size(tmp,1); %GIACOMO, preallocate space for filt_ker variable
filt_ker=zeros(size(tmp,1), size(conv(tmp(i,:),normpdf(1:x_scal,x_scal/2,x_scal/20),'same')/x_scal,2));
for i=1:size(tmp,1)
...
if PC<7 %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!era isempty(PC)
%tmpfig = figure(666); GIACOMO,commented
...
subplot(1,2,1);
tmpsig = U(:,PC)'*data; %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! invece di PC era i
%        newtimef( tmpsig, length(tmpsig), [timeAxis(1) timeAxis(end)], fs, 0 );
 [tf,freqs,~] = timefreq(tmpsig,fs,'freqs',[0 300]); %GIACOMO chenged [0 500] to [0 300]
 imagesc(timeAxis,freqs,abs(tf)); %here it is possible to change the color limit 
 size(tf)
 colorbar %GIACOMO
% ylim([0,200])
...
subplot(1,2,2);
 plot(diag(S))
 hold on;
 plot(PC,S(PC,PC),'r.','MarkerSize',10) %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! invece di PC era i
 xlabel('Channels');  %GIACOMO, add axes label
 ylabel('Variance');
 hold off;
 
 PCs_variance=S(PC,PC); % GIACOMO, store this varibale and output it. This make up the list of suggested PCs to remove
...
% 
% close(tmpfig); %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! fino a qui



PC = PC -1;   %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! invece di PC era i
M = rank(data) -PC;
end

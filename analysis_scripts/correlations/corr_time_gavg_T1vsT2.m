...
%% define stat dir
TEPs_stat_dir='Z:\Giacomo_20190828\Analysis\Post_processing\TEPs\NEW_postprocessing\Correlation_dot_product_NEW\T1vsT2\time';
...
for area_count=1:length(area)
    ...
    for pip_count=1:length(pipeline)
        ...
        for sub_count=1:16
            ...
            parfor chan_count=1:chan_num
                selected_data1 =squeeze(struct2array(data_gavg_ind{pip_count,area_count,1}).individual(sub_count,chan_count,:));
                selected_data2 =squeeze(struct2array(data_gavg_ind{pip_count,area_count,2}).individual(sub_count,chan_count,:));
        ...
        % test significance
        [H_spear(pip_count,:),PvalSPEAR(pip_count,:)]= ttest(z_RHO, 0, 'alpha', 0.025, 'dim', 1, 'tail', 'both');
        ...
        savefig([TEPs_stat_dir '\corrspearSES_TIME_correct_NEW_'...
        ...
        saveas(gcf,...
            [TEPs_stat_dir '\corrspearSES_TIME_correct_NEW_'...
            pipeline_short{pip_count} '_'...
            pipeline_short{pip_count} '_'...
            area{area_count} '_'...
            num2str(interval(1)*1000) '_'...
            ...
            'tif');
        % save
        save([TEPs_stat_dir '\stat_corrspearSES_TIME_correct_NEW_'...
            pipeline_short{pip_count} '_'...
            pipeline_short{pip_count} '_'...
            area{area_count} '_'...
            num2str(interval(1)*1000) '_'...
